...
namespace Sop.Data.Events
{
    ...
    public class EventBus<S, T> : IEventBus<S, T> where T : CommonEventArgs
    {
        ...
        //private static readonly ILog logger = LogManager.GetLogger<EventBus<S, T>>();
        private static readonly ILogger logger = new LoggerConfiguration()
            .MinimumLevel.Verbose() 
            .CreateLogger()
            .ForContext<EventBus<S, T>>();
        ...
        public void OnBefore(S sender, T eventArgs)
        {
            ...
            if (handler != null)
            {
                ...
                foreach (CommonEventHandler<S, T> dele in delegateArray)
                {
                    try
                    {
                        dele.BeginInvoke(sender, eventArgs, InvokeCallback, dele);
                    }
                    catch (Exception e)
                    {
                        logger.Error("执行触发操作执行前事件时发生异常", e);
                    }
                }
            }
        }
        ...
        public void OnAfter(S sender, T eventArgs)
        {
            ...
            if (handler != null)
            {
                ...
                foreach (CommonEventHandler<S, T> dele in delegateArray)
                {
                    ...
                    {
                        logger.Error("执行触发操作执行后事件时发生异常", e);
                    }
                }
            }
        }
        ...
        public void OnBeforeWithHistory(S sender, T eventArgs, S historyData)
        {
            ...
            if (handler != null)
            {
                ...
                foreach (EventHandlerWithHistory<S, T> dele in delegateArray)
                {
                    try
                    {
                        dele.BeginInvoke(sender, eventArgs, historyData, InvokeCallback, dele);
                    }
                    catch (Exception e)
                    {
                        logger.Error("执行触发含历史数据操作执行前事件时发生异常", e);
                    }
                }
            }
        }
        ...
        public void OnAfterWithHistory(S sender, T eventArgs, S historyData)
        {
            ...
            if (handler != null)
            {
                ...
                foreach (EventHandlerWithHistory<S, T> dele in delegateArray)
                {
                    ...
                    {
                        logger.Error("执行触发含历史数据操作执行后事件时发生异常", e);
                    }
                }
            }
        }
        ...
        public void OnBatchBefore(IEnumerable<S> senders, T eventArgs)
        {
            ...
            if (handler != null)
            {
                ...
                foreach (BatchEventHandler<S, T> dele in delegateArray)
                {
                    try
                    {
                        dele.BeginInvoke(senders, eventArgs, InvokeCallback, dele);
                    }
                    catch (Exception e)
                    {
                        logger.Error("执行触发批量操作执行前事件时发生异常", e);
                    }
                }
            }
        }
        ...
        public void OnBatchAfter(IEnumerable<S> senders, T eventArgs)
        {
            ...
            if (handler != null)
            {
                ...
                foreach (BatchEventHandler<S, T> dele in delegateArray)
                {
                    ...
                    {
                        logger.Error("执行触发批量操作执行后事件时发生异常", e);
                    }
                }
            }
        }
